...
import 'package:bus_book/models/bus_name_cards_without_login.dart';
...
class Home_without_login extends StatefulWidget {
  const Home_without_login({super.key});

  @override
  State<Home_without_login> createState() => _Home_without_loginState();
}

class _Home_without_loginState extends State<Home_without_login> {
  ...
  Future<void> bus_details_get(
      String source, String destination, String date) async {
    var url = Uri.parse(base_url + 'search_bus/' + source + '/' + destination + '/' + getWeekday(date).toString() +
'/' +
        date);
    ...
    if (response.statusCode == 200) {
      ...
      Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) => bus_name_cards_without_login(
                    busRoutes: busRoutes,
                    source: source,
